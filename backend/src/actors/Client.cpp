...
void Client::init(int world_rank, int world_size) {
	...
	while (true) {
		...
		std::vector<int> data2(tileInfo.xRes * tileInfo.yRes);
        ...
        for (int x = 0 ; x < tileInfo.xRes ; x++) {
			for (int y = 0 ; y < tileInfo.yRes ; y++) {
				...
				data2[x + y * tileInfo.xRes] = f->calculateFractal((tileInfo.minReal + x * deltaReal), (tileInfo.minImaginary + y * deltaImaginary), tileInfo.maxIteration);
			}
		}
		...
		MPI_Send((const void*)&data2[0], tileInfo.xRes * tileInfo.yRes, MPI_INT, 0, 2, MPI_COMM_WORLD);
    }
}

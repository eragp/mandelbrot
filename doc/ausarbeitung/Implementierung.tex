...
% Let's remove this part. It's being handled in Host transparently and only a minor issue
...
Dabei wird der momentan sichtbare Ausschnitt der Menge mit Hilfe der WebSockets Verbindung (\autoref{sec:fontend_communication}) an das Backend versendet und vom ausgewählten Lastbalancierer in Teilregionen unterteilt (\autoref{sec:load_balancing}). 
...
\subsubsection{Anzeigen der Regionsdaten} % tileDisplay/

\subsubsection{Darstellung der Rechenzeiten} % components/

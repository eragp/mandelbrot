...
Wie in \autoref{fig:concept_coordinates} zu sehen, ist dabei der horizontale Offset und vertikale Offset
die linke obere Koordinate der Region in Pixeln.
...
% TODO dokumentieren, dass regionsaufträge erst nach der information des frontends  gesendet werden (falls nicht-filtern gemerged wird)
...
\begin{figure}
	\lstinputlisting[caption={Ausschnitt aus den Daten einer versendeten Teilregion. Punkt (x,y) liegt in \texttt{"data"} an Index $i=x+y*width$.}, label={src:regionData.json}, language=json]{regionData.short.json}
\end{figure}
...
% TODO dokumentieren des filterns von regionsdaten, die nicht in aktueller region liegen (falls merge)

...
Die Methode Host::start\_server initialisiert dabei lediglich den Websocketserver mit den Methoden zum Behandeln geöffneter und geschlossener Verbindungen und
einer Methode um Nachrichten des Clients zu behandeln, Host::handle\_region\_request.
...
Diese Methode versucht, den Inhalt der empfangenen Nachricht als JSON zu dekodieren und entnimmt die für das struct Region
notwendigen Werte unter gleichem Namen dem Objekt, das unter dem Schlüssel "region" in der geparsten Anfrage gespeichert ist.
Außerdem erwartet es unter dem Schlüssel "balancer" einen String, der den zu wählenden Lastbalancierer bestimmt und unter dem Schlüssel "type" den String "regionRequest".
Mögliche Zeichenketten hierfür sind in den Klassen der Balancierer unter backend/src/balancer in der globalen Variable
\textit{Klassenname}::NAME gespeichert. Ein Beispiel für einen Namen kann in \autoref{src:PredictionBalancer.NAME} gefunden werden:
...
Nach dem Parsen der Nachricht wird die Region global festgelegt und beim korrekten Lastbalancierer eine Zerlegung der angefragten Region über \textit{balanceLoad} gestartet.
...
gelegt. Der MPI verwendende Thread, der diese Regionen anschließend an die Worker sendet wird über das Setzen eines boolschen Wertes
auf \textit{true} darüber informiert, dass neue Regionen zum versenden zur Verfügung stehen.
Dabei ist der Rang des Workers, der eine Region berechnen soll genau der Index der Region in der Datenstruktur sein.
Ist ein Worker nicht verfügbar, so werden alle folgenden Ränge um eins erhöht.
...
\begin{figure}
    \lstinputlisting[caption={Eine gültige Antwort auf die Region aus \autoref{src:regionRequest.json} in JSON}, label={src:region.json}, language=json]{region.json}
\end{figure}

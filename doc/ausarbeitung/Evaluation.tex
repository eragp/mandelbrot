...
\subsection{Evaluierung der MPI-Kommunikation}

Im Folgenden wird untersucht, wie sich die MPI-Kommunikation bei einer Erhöhung der Workeranzahl verhält.

\subsubsection{Skalierung der MPI-Kommunikation}

..... NOCH NICHT GANZ KLAR, WARUM DAS SO IST.....
\\
\\
...
\subsubsection{Optimale Anzahl an Workern für Standardimplementierung}
...
Bei der verteilten Berechnung ist ein wesentlicher Aspekt, dass der MPI-Kommunkationsoverhead im Verhältnis zur Rechenzeit je Worker nicht zu groß wird und damit die Gesamtperformance schlechter wird als bei der Verwendung von weniger Workern. Je mehr Worker verwendet werden, desto geringer wird die benötigte Rechenzeit bei in etwa gleichbleibender Kommunikationszeit. Die optimale Anzahl an Workern ist erreicht, wenn \( MPIKommunikationszeit + Rechenzeit \) minimal ist. Diese optimale Anzahl an Workern ist auf jeden Fall überschritten, wenn die MPI-Kommunikationszeit größer ist als die Rechenzeit.
\\
\\
In \autoref{fig:scaleGraph} ist für jeden Regionstyp (Außen-, Rand-, Innenregion) die MPI-Kommunikationszeit und die maximale Berechnungszeit pro Anzahl an Workern dargestellt. Dabei wurde in den Workern keine weitere Parallelisierung (weder mit OpenMP, noch mit SIMD) vorgenommen.

Die Erwartung ist, dass für die optimale Anzahl an Workern die Relation \( Aussenregionen \leqslant Randregionen \leqslant Innenregionen \) gilt, da die selbe Relation auch für die Rechenzeiten der Regionstypen gilt und damit der MPI-Kommunikationsoverhead bspw. für Außenregionen früher eine signifikante Rolle spielt als bei den anderen Regionstypen.
\\
\\
In \autoref{fig:scaleGraph} ist zu erkennen, dass die optimale Anzahl an Workern für Außenregionen bei xx liegt. Für Rand- und Außenregionen liegt diese Anzahl bei 40, was die maximale Anzahl an zur Verfügung stehenden Workern ist. Es kann angenommen werden, dass diese Anzahl noch größer sein würde, ständen mehr Worker zur Verfügung. Damit stimmen die Messergebnisse mit den Erwartungen überein.
%TODO Noch Strich in der Grafik einfügen für Minimum von MPI-Kommunikationszeit + Rechenzeit

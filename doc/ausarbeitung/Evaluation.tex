...
Generell lässt sich beobachten, dass die MPI-Kommunikationszeit nur sehr gering
(um ca. \( 50ms \)) pro Regionstyp bei steigender Anzahl an Rechenknoten fluktuiert.
Sie wird daher für die folgende Evaluierung als nahezu konstant angenommen.
...
\end{figure}
\paragraph{Optimale Anzahl an Workern für Standardimplementierung}
Bei der verteilten Berechnung ist ein wesentlicher Aspekt, dass der MPI-Kommunikationsoverhead
im Verhältnis zur Rechenzeit je Worker nicht zu groß wird und damit die Gesamtperformance schlechter wird,
als bei der Verwendung von weniger Rechenknoten.
Je mehr Knoten verwendet werden, desto geringer wird die benötigte Rechenzeit bei in etwa gleichbleibender Kommunikationszeit \( t \approx c / n \).
Die optimale Anzahl an Konten ist dabei erreicht, wenn \( \text{MPIKommunikationszeit} + \text{Rechenzeit} \) minimal ist.

In \autoref{fig:scaleGraph} ist für jeden Regionstyp (Außen-, Rand-, Innenregion) die MPI-Kommunikationszeit
und die maximale Berechnungszeit pro Anzahl an Knoten dargestellt, wobei keine weitere Parallelisierung, wie OpenMP oder SIMD verwendet.
Die Erwartung ist dabei, dass für die optimale Anzahl an Knoten die Relation \( \text{Aussen-} \leq \text{Rand-} \leq \text{Innenregionen} \) gilt,
da die selbe Relation auch für die Rechenzeiten der Regionstypen gilt und
damit der MPI-Kommunikationsoverhead bspw. für Außenregionen früher eine signifikante Rolle spielt als bei
den anderen Regionstypen.

In \autoref{fig:scaleGraph} ist zu erkennen, dass die optimale Anzahl an Rechenknoten für Außenregionen bei monoton fällt.
Somit ist anzunehmen, dass die maximale Rechenzeit mit zunehmender Anzahl an Rechenknoten fallen würde, womit das Optimum
für das HimMUC cluster möglichst groß gewählt werden sollte, da die MPI Zeit im vergleich zur Rechenzeit vernachlässigbar ist.
